...
class TextLayoutRenderer {
    ...
    private func boundsForBackground(
        segment: LineSegment,
        line: Line,
        type: TextBackgroundBoundsAttribute
    ) -> UIRectangle {
        switch type {
        case .segmentBounds:
    ...
    private func toBLImage(_ image: any Image) -> BLImage {
        guard let image = image as? Blend2DImage else {
            fatalError("Unknown font type \(type(of: image)), expected type \(Blend2DImage.self)")
        }

        return image.image
    }

    private func toBLFont(_ font: any Font) -> BLFont {
    ...
    private func toBLGlyphBuffer(_ buffer: any GlyphBuffer) -> BLGlyphBuffer {
